<template>
  <div class="w-full bg-[rgb(248,248,248)]">
    ...
    >
      <div class="flex h-full flex-col bg-white shadow-md relative">
        ...
        <div class="sticky top-0 bg-white z-10 pt-4">
          ...
          <NuxtLink
            to="/"
            class="flex mt-2 items-center px-4 pb-6 cursor-pointer"
            @click="closeSidebar"
          >
            <img
              class="h-12 w-auto max-w-full align-middle"
        ...
        <!-- Nav scrollable -->
        <div
          class="flex flex-1 flex-col pb-10 overflow-y-auto scrollbar-hidden"
        >
          <nav class="flex-1">
            <template v-for="(item, index) in menuItems" :key="index">
              <span
                v-if="item.type === 'divider'"
                class="ml-3 mt-4 mb-2 block text-xs font-semibold text-gray-500"
              >
                {{ item.label }}
              </span>
    ...
    <div class="lg:ml-64 min-h-screen">
      <nav
        class="sticky top-0 right-0 left-0 lg:left-64 bg-white shadow-sm z-20"
      >
        <div class="px-6 py-4">
...
const userAvatar = computed(
  () =>
    `https://ui-avatars.com/api/?name=${encodeURIComponent(userName.value)}&background=60a5fa&color=fff`,
);
...
const getLinkClass = (path) => {
  const baseClasses =
    "flex cursor-pointer items-center py-3 px-4 text-sm font-medium outline-none transition-all duration-100 ease-in-out";
